...
function FireChatSettingsFiles<M extends FireMessage<T>, T extends FireMessageContent>({
    fileMessages,
}: {
    fileMessages: M[];
}) {
    ...
    return (
        <div className="flex flex-col gap-4">
            {fileMessages.length > 0 ? (
                    ...
                    {[...fileMessages]
                        ...
                        .map((msg, index) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={cn(`py-2.5 px-3 flex items-center group/file`, {
                                        "border-t": index !== 0,
                                    })}
                                >
                                    <div className="text-sm font-medium text-foreground transition-colors line-clamp-1">
                                        {truncateFilenameMiddle(
                                            message[MESSAGE_CONTENTS_FIELD][0][MESSAGE_CONTENT_FILE_NAME_FIELD] ?? "",
                                            20,
                                        )}
                                        <span className="text-sm text-muted-foreground ml-2">
                                            {formatSizeString(
                                                message[MESSAGE_CONTENTS_FIELD][0][MESSAGE_CONTENT_FILE_SIZE_FIELD] ??
                                                    0,
                                            )}
                                        </span>
                                    </div>
                                    <Button
                                        ...
                                        onClick={async () => {
                                            console.log(
                                                message[MESSAGE_CONTENTS_FIELD][0][MESSAGE_CONTENT_URL_FIELD] as string,
                ...
                <span className="text-sm text-muted-foreground text-center py-6">{FIRE_CHAT_LOCALE.NO_FILES}</span>
            )}
