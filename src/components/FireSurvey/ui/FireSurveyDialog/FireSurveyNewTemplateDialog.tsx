...
import type { SurveyType, QuestionType } from '../../settings/types';
import { useFireAuth } from '@/components/FireProvider/FireAuthProvider';
import { FireUser } from '@/lib/FireAuth/settings';

interface FireSurveyNewTemplateDialogProps {
    open: boolean;
...
export function FireSurveyNewTemplateDialog({
    open,
    onOpenChange,
}: FireSurveyNewTemplateDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success(FIRE_SURVEY_LOCALE.SUCCESS.CREATED);
            onOpenChange(false);
            resetForm();
        } catch {
            toast.error(FIRE_SURVEY_LOCALE.ERRORS.CREATE_FAILED);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const resetForm = () => {
        setTitle('');
        setDescription('');
        setQuestionType(QUESTION_TYPE_TEXT);
        setIsRequired(false);
        setTargetTypes([]);
        setIsActive(true);
        setOptions(['', '']);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{FIRE_SURVEY_LOCALE.TEMPLATE.NEW}</DialogTitle>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={isLoading}>
                            {FIRE_SURVEY_LOCALE.BUTTONS.CREATE}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
